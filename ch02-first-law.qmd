...
::: {.gap height="35mm" hint="$P$--$V$ axes; mark the initial and final states, join them with any squiggly path, label it \"process\"."}
...
::: {.gap height="39mm" hint="Box of gas, cut the heat, isolate it, let time pass until it is uniform. Then: thermal + mechanical + chemical equilibrium = thermodynamic equilibrium."}
...
::: {.gap height="43mm" hint="Two states on a $P$--$V$ plot, joined by two different paths $a$ and $b$. Same end states --- but $Q_a \neq Q_b$ and $W_a \neq W_b$."}
...
::: {.gap height="27mm" hint="$\mathrm{d}W = \vec{F}\cdot\mathrm{d}\vec{x}$ (dot product --- only the component of $F$ along $\mathrm{d}x$ counts), then integrate."}
$$
...
::: {.gap height="25mm" hint="ON the system by the surroundings = compression = $W > 0$. BY the system on the surroundings = expansion = $W < 0$."}
| | |
|:--|:--|
...
::: {.gap height="45mm" hint="$\mathrm{d}W = F_\text{ext}\,\mathrm{d}l$ and $P_\text{ext} = F_\text{ext}/A$ give (I). Geometry gives (II) $\mathrm{d}V = -A\,\mathrm{d}l$. Combine, then integrate."}
$$
...
Combining (I) and (II), and integrating:
$$
...
::: {.gap height="31mm" hint="$W = -\int P_\text{ext}\,\mathrm{d}V$, so $|W|$ is the area under the path in the $P_\text{ext}$--$V$ diagram."}
...
::: {.gap height="47mm" hint="$P_\text{ext}$ is constant so it comes out of the integral. Sketch the rectangle on the $P_\text{ext}$--$V$ diagram. Expansion, so $W < 0$."}
...
::: {.gap height="16mm" hint="Shaft power = torque $\times$ rotational speed."}
$$
...
::: {.gap height="16mm" hint="Electrical power = voltage $\times$ current."}
$$
...
![](figures/img/gas-hotplate.png){.wrap width=26%}
...
![](figures/img/circuit-analogy.png){.wrap width=28%}
...
::: {.gap height="16mm" hint="Left: boundary around the whole box, nothing crosses it, $Q = 0$. Right: boundary around the cold part only, $Q \neq 0$."}
...
::: {.gap height="43mm" hint="$W_\text{cycle} = W_{12} + W_{21}$; the two areas partly cancel and what survives is the enclosed area."}
...
= \text{area enclosed by the cycle}
$$
:::
...
::: {.gap height="35mm" hint="Careful: the horizontal axis is SPECIFIC volume, so multiply by the mass. Topmost path runs left to right (expansion), so $W_\text{cycle} < 0$."}
...
::: {.gap height="31mm" hint="Write $\sum W + \sum Q = 0$ around a closed loop on the $P$--$V$ diagram; the sums mean 'energy exchanged with the surroundings'."}
$$
\sum W + \sum Q = 0
...
::: {.gap height="25mm" hint="$W + Q$ on the left is energy TRANSFER; $U_2 - U_1$ on the right is energy CHANGE."}
$$
...
::: {.gap height="23mm" hint="Two paths $a$ and $b$: $W_a \neq W_b$ and $Q_a \neq Q_b$, but both sums equal $U_2 - U_1$."}
$$
W_a + Q_a = U_2 - U_1
\qquad\text{and}\qquad
W_b + Q_b = U_2 - U_1
...
::: {.gap height="17mm" hint="Divide the whole equation by the mass: capital letters [J] become lower-case [J/kg]."}
$$
...
::: {.gap height="66mm" hint="Part 1: get $U_b - U_a$ from the $acb$ path first, then reuse it for $adb$. Part 2 has two routes --- the non-cyclic form $b \to a$, or the cyclic form round $acb + ba$. Show both, they must agree."}
**Part 1.** Work done *by* the system is negative in our sign convention, so
...
**Part 2, route I (non-cyclic form).** Work done *on* the system is positive, so
...
**Part 2, route II (cyclic form).** The path $a \to c \to b \to a$ is a complete
...
::: {.gap height="20mm" hint="$KE = \tfrac{1}{2}mC^2$, $PE = mgz$."}
$$
...
::: {.gap height="23mm" hint="(a) boundary round the gas only: $Q \neq 0$, $W = 0$. (b) boundary round gas + copper plate: $Q = 0$, $W \neq 0$. (c) boundary round everything: $Q = 0$ and $W = 0$."}
...
The problems below are the ones that used to be handed out as the Week 3 and
Week 4 sheets. Difficulty is marked **[easy]**, **[medium]** or **[hard]**.
Work through them once the chapter is finished; one of them will be solved
together in class.

## Work

::: {.activity title="In-class activity"}
**P5** below is the one we will do together in class. Try it first on your own,
then compare.
:::
...
**P2 [medium].** A stop is located 20 mm above the piston at the position shown.
...
**P3 [medium].** The refrigerant R-410a contained in the cylinder is at 1000 kPa,
...
**P4 [medium].** A 200 mm diameter piston compresses the air inside a cylinder by
...
**P5 [medium] — worked together in class.** A gas contained within a
...
First, $P_2V_2 = P_3V_3$, so
$P_3 = P_2\left(V_2/V_3\right) = 2 \times \tfrac{4}{2} = 4$ bar.

**Process 1–2:** $V$ is constant, the piston does not move, so
$$W_{12} = 0$$

**Process 2–3:** $P_\text{ext}V = C$:
$$
...
**P6 [hard].** Warm air is contained in a piston–cylinder assembly oriented
...
::: {.activity title="In-class activity"}
**P8** below is the one we will do together in class — it is the one that forces
you to use the *property* nature of $U$ rather than just plugging numbers in.
:::

**P7 [easy].** As shown in the figure, 5 kg of steam contained within a
...
**P8 [medium] — worked together in class.** The following table gives data, in
...
To find $Q_{23}$ we first need $\Delta U_{23}$. For the whole **cycle**,
$\Delta U = 0$, because the initial and final states of the cycle are the same.
From the 4–1 row, $\Delta U_{41} = 500 + 700 = 1200$ kJ, so
$$
...
**P9 [easy].** A closed system of mass 5 kg undergoes a process in which there is
...
**P10 [easy].** A closed system of mass 20 kg undergoes a process in which there
...
**P11 [medium].** Four kilograms of carbon monoxide (CO) is contained in a rigid
...
**P12 [hard].** Gaseous CO$_2$ is contained in a vertical piston–cylinder assembly
